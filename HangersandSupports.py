...
# 2. EXCEL DATABASE HEADERS (Must match the columns on your 'Hangers & Supports List' tab)
COL_CATALOG = "type"
...
STOP_WORDS = {'THE', 'AND', 'FOR', 'WITH', 'ASSORTED', 'SIZE', 'SIZES', 'HANGER', 'SUPPORT', 'STRUT', 'ANCHOR'}
...
def embed_pdfs_into_excel(wb, pdf_paths_to_embed, sheet_index_obj):
    index_sheet_name = 'Hangers and Supports Index'
    cut_sheet_name = 'Hangers and Supports Cut Sheets'
...
def extract_hangers_section(full_spec_text):
    return extract_spec_section(
        full_spec_text,
        'Hangers and Supports',
        start_patterns=[
            r'SECTION\s+\d{2}\s*\d{2}\s*\d{2}[^\n]*HANGERS\s+AND\s+SUPPORTS',
            r'SECTION\s+\d{2}\s*\d{2}\s*\d{2}[^\n]*SUPPORTING\s+DEVICES',
            r'HANGERS\s+AND\s+SUPPORTS\s+FOR\s+ELECTRICAL',
            r'SECTION\s+26\s*05\s*29'
        ],
        end_patterns=[
            r'SECTION\s+\d{2}\s*\d{2}\s*\d{2}',
...
def get_spec_header(spec_text):
    ...
    return "HANGERS AND SUPPORTS FOR ELECTRICAL SYSTEMS"
...
try:
    ...
    try:
        ...
        sheet_index = None
        for name in ['Hangers and Supports Index', 'Hangers & Supports Index']:
            try:
        ...
        if name_in_a5 == "None" or not name_in_a5:
            name_in_a5 = "Hangers and Supports"

    except Exception as e:
        ask_to_continue("Reading 'Hangers and Supports Index' sheet", e)
        name_in_a5 = "Hangers and Supports"
    ...
    hangersandsupportsPDF = os.path.abspath(os.path.join(PROJECT_FOLDER, pdf_filename))
    ...
    if JOB_FORM_PDF_NAME and "ERROR" not in JOB_FORM_PDF_NAME:
        ...
        try:
            ...
            # Call AI strictly using the prompt and text parameters
    ...
    try:
        ws_list = None
        for name in ['Hangers & Supports List', 'Hangers and Supports List']:
            try:
        ...
        if ws_list is None:
            raise Exception("Could not find the 'Hangers & Supports List' tab in Excel.")
    ...
    wiring_spec_text = extract_hangers_section(full_spec_text)

    # --- EXTRACT SPEC HEADER & WRITE TO A5 ---
    spec_title = get_spec_header(wiring_spec_text)

    # NEW FORMATTING: Force XX XX XX format and remove redundant ALL CAPS text
    spec_num_match = re.search(r'(\d{2}\s*\d{2}\s*\d{2})', spec_title)
    spec_num = format_spec_number(spec_num_match.group(1)) if spec_num_match else "Unknown"

    if spec_num != "Unknown":
        final_a5_string = f"{spec_num} - {name_in_a5}"
    else:
    ...
    try:
        ...
        safe_write_cell(ws_review, 11, 'B', f"Spec Section No: {spec_num}")
        safe_write_cell(ws_review, 15, 'B', f"Submittal Title: {name_in_a5}")
    ...
    combined_project_data = (
        ...
        f"--- HANGERS AND SUPPORTS SPECIFICATION SECTION ---\n{wiring_spec_text}\n\n"
...
HANGERS AND SUPPORTS SPECIFIC RULES (CRITICAL MANDATE):
1. BUILDING ENVIRONMENT & STRUCTURE DEDUCTION (CRITICAL): You MUST analyze the Contract, Drawings, and Project Name to deduce the building type and environment (e.g., 'Parking Garage' = concrete, damp, potentially corrosive; 'Office' = dry, drop ceilings).
   - MATERIAL FINISH: Based on the deduced environment, select the appropriate material finishes from the database. For example, a Parking Garage is a damp/corrosive environment and strictly requires Hot-Dipped Galvanized, PVC-coated, or Stainless Steel hardware. DO NOT pull standard indoor zinc-plated hardware for outdoor/garage environments.
   - ANCHORS: If it is a CONCRETE structure, you MUST pull Concrete Anchors (e.g., Wedge Anchors, Tapcons). If it is a STEEL structure, you MUST pull Steel Fasteners (e.g., Beam Clamps). If mixed, pull both.
2. UNISTRUT KITTING (MANDATORY): If you determine Strut Channel (which may also be called "Metal Channel", "Unistrut", or "Strut Channel") is required or allowed, you MUST completely kit the assembly. You are strictly mandated to pull Spring Nuts and Threaded Rod from the database alongside the strut. Ensure the finish of the nuts/rod matches the strut finish.
3. FASTENER RESTRICTIONS: Actively scan the documents to see if Powder-Actuated fasteners are permitted or explicitly banned. If explicitly banned, DO NOT pull them. Also, look for drop ceilings; if present, pull Ceiling Grid Wire.
4. MC CABLE SUPPORTS: Scan the Contract and Drawings to determine if MC Cable is being utilized on the project. If MC Cable is permitted or used, you MUST submit MC Snap Clips and associated MC support hardware from the database.
5. THE "MISSING SPEC" PROTOCOL: If the explicit "Hangers and Supports" specification section is missing or extremely brief, you must act as an expert estimator. Deduce the required hangers, supports, and anchors based strictly on the Contract Scope of Work, the raceways you know are being used, and the type of building structure/environment.

SPEC/CONTRACT CITATIONS (MANDATORY):
You must find and record the exact Specification Section, Page Number, OR Contract Clause for the specific rules and thresholds you apply. Put this information in the "Spec_Citation" field for the relevant item. If you deduced the requirement based on Building Type or Missing Specs, note "Deduced from Contract Scope/Building Type".
...
"""
    ...
    # --- CALL GEMINI ---
    print("\nSending Contract, Hangers & Supports specs, and drawing notes to Gemini...")
    extracted_data = []
    try:
        # Call AI strictly using the prompt and data parameters
    ...
    if embedded_paths_list:
        ...
        print(f"\n✅ Embedded {success_count} cut sheet PDF(s) into 'Hangers and Supports Cut Sheets' tab.")
    else:
    ...
    try:
        print(f"Saving final submittal to: {hangersandsupportsPDF}")
        saved = save_pdf_with_retry(merged_doc, hangersandsupportsPDF)
        merged_doc.close()

        if saved:
            print(f"✅ Master Submittal created: {hangersandsupportsPDF}")
            print("Launching final document...")
            os.startfile(hangersandsupportsPDF)
        else:
