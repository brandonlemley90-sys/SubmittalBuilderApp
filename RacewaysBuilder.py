...
# 2. EXCEL DATABASE HEADERS (Must match the columns on your 'Raceways List' tab)
COL_CATALOG = "type"
...
STOP_WORDS = {'THE', 'AND', 'FOR', 'WITH', 'ASSORTED', 'SIZE', 'SIZES', 'CONDUIT', 'FITTING', 'EMT', 'PVC'}
...
def embed_pdfs_into_excel(wb, pdf_paths_to_embed):
    index_sheet_name = 'Raceways Index'
    cut_sheet_name = 'Raceways Cut Sheets'
...
def extract_raceways_section(full_spec_text):
    return extract_spec_section(
        full_spec_text,
        'Raceways and Boxes',
        start_patterns=[
            r'SECTION\s+26\s*05\s*33',
            r'SECTION\s+16\s*11\s*10',
            r'RACEWAY[S]?\s+AND\s+BOXES',
            r'CONDUIT[S]?\s+AND\s+FITTING[S]?',
            r'RACEWAY[S]?'
        ],
        end_patterns=[
...
def get_spec_header(spec_text):
    ...
    return "26 05 33 - RACEWAYS AND BOXES FOR ELECTRICAL SYSTEMS"
...
try:
    ...
    try:
        sheet_index = wb.sheets['Raceways Index']
        ...
        ask_to_continue("Reading 'Raceways Index' sheet", e)
        name_in_a5 = "Raceways"
    ...
    racewaysPDF = os.path.abspath(os.path.join(PROJECT_FOLDER, pdf_filename))
    ...
    try:
        ws_list = wb.sheets['Raceways List']
        sheet_data = ws_list.used_range.value

        if not sheet_data:
            raise Exception("The 'Raceways List' sheet appears to be empty.")
        ...
        print(f"\nLoaded {len(db_for_prompt)} official raceway items from Excel.")
    except Exception as e:
    ...
    wiring_spec_text = extract_raceways_section(full_spec_text)
    ...
    combined_project_data = (
        ...
        f"--- CONDUIT AND RACEWAYS SPECIFICATION SECTION ---\n{wiring_spec_text}\n\n"
...
2. SPECIFIC SWAP ENFORCEMENT (CONTEXT IS KEY): If a Scope Alternate substitutes a material (e.g., GRC in lieu of Schedule 80 PVC), apply this swap ONLY to the intended applications. DO NOT blanket-ban a material (like PVC) if it is still explicitly required for other specific uses like UNDERGROUND or IN-SLAB installations.
...
CONDUIT AND FITTING SPECIFIC RULES (CRITICAL MANDATE):
1. COMPLETE FITTING KITTING (CRITICAL): When you select a conduit type (like GRC, IMC, EMT, or PVC), you MUST pull ALL of its associated fittings and accessories from the database. Do not just pull the conduit. For GRC/IMC, actively hunt the database for Rigid Couplings, Nipples, Locknuts, Grounding Bushings, Insulated Bushings, Threaded Hubs (Meyers Hubs), Erickson Couplings, and Sealing Fittings. If the spec requires them, pull EVERY matching component.
2. UNDERGROUND & IN-SLAB RACEWAYS: You MUST actively scan the specs for underground, earth, direct-burial, or concrete-encased requirements. If Schedule 40 PVC, Schedule 80 PVC, or PVC-coated conduit is specified for underground use, you MUST pull it and its associated PVC fittings from the database. Do not skip underground items!
3. MATERIAL SPECIFICATIONS: Actively scan the documents to determine which conduit types are allowed for above-ground, concealed, and exposed work (e.g., EMT, RMC/GRC, IMC, FMC, LFMC/Sealtite). Apply overrides from the Scope Alternates.
4. EXPANSION COUPLINGS (MANDATORY): You MUST actively search the specs and drawings for "Expansion Couplings", "Expansion Joints", "Expansion Fittings", or "Deflection Fittings". If the documents mention conduit crossing expansion joints, you are STRICTLY REQUIRED to pull the Expansion Coupling from the database. DO NOT SKIP THIS.
5. FITTING TYPES (EMT & GRC): If EMT or GRC is specified, you MUST determine if the fittings are required to be Compression or Set-Screw. You must also determine if they are required to be Steel or Malleable Iron vs Die-Cast, and Insulated Throat or Non-Insulated. Pull the exact corresponding fittings from the database.

SPEC/CONTRACT CITATIONS (MANDATORY):
You must find and record the exact Specification Section, Page Number, OR Contract Clause for the specific rules and thresholds you apply. Put this information in the "Spec_Citation" field for the relevant item. If the decision was based on a Scope Alternate overriding the Spec, explicitly note that.
...
"""
    ...
    # --- CALL GEMINI ---
    print("\nSending Contract, Raceways specs, and drawing notes to Gemini...")
    extracted_data = []
    try:
    ...
    if embedded_paths_list:
        ...
        print(f"\n✅ Embedded {success_count} cut sheet PDF(s) into 'Raceways Cut Sheets' tab.")
    else:
    ...
    try:
        print(f"Saving final submittal to: {racewaysPDF}")
        saved = save_pdf_with_retry(merged_doc, racewaysPDF)
        merged_doc.close()

        if saved:
            print(f"✅ Master Submittal created: {racewaysPDF}")
            print("Launching final document...")
            os.startfile(racewaysPDF)
        else:
