...
def save_pdf_with_retry(merged_doc, output_path, max_attempts=3):
    ...
    for attempt in range(1, max_attempts + 1):
        if check_file_locked(output_path):
            print(f"\n⚠️  OUTPUT FILE IS LOCKED")
            print(f"   Close '{output_path}' in Bluebeam/Acrobat.")
...
# =============================================================================
try:
    if os.environ.get("RUNNING_FROM_META") != "TRUE":
        print("\n" + "=" * 50)
        API_KEY = web_prompt("TEXT", "🔑 Please enter your Gemini API Key: ")
        print("=" * 50 + "\n")
    else:
            ...
            if not pdf_paths:
                ...
                if fallback_paths:
                    ...
                    if f_prefix or f_score >= 60:
                        ...
                        ans = web_prompt("YN", f"Do you want to use the base model '{best_name}' PDF instead? (Y/N): ").upper()
                        if ans == 'Y':
                            pdf_paths = fallback_paths
                        else:
